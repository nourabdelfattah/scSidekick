...
PlotDimPlots <- function(seurat_object,
  ...
  .make_row_combo <- function(i) {
    ...
    if (show_trend) {
      # Flat list: trend_plot + every UMAP panel in this row
      all_panels <- c(list(trend_row_plots[[i]]), umap_panels)
      pw <- patchwork::wrap_plots(all_panels, nrow = 1L,
                                   widths = c(trend_width, rep(1L, max_cols)))
    } else {
      pw <- patchwork::wrap_plots(umap_panels, nrow = 1L)
    }

    # Row label on the right when row.by is active
    if (has_row)
      pw <- ggpubr::annotate_figure(
        pw, right = ggpubr::text_grob(rl, rot = 270, face = "bold", size = 14)
      )
    pw
  }
  ...
  if (!is.null(output_dir)) {
    ...
    message("scSidekick: Saved to ", fpath,
            " (", round(pdf_w, 1), " x ", round(pdf_h, 1), " in)")
    .write_legend_sidecar(fpath, paste0(
      toupper(reduction), " plot coloured by ", group.by,
      ...
      ". A single shared colour legend is shown below the panels",
