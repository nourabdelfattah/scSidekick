...
#' @param cluster_colors Named character vector of colors for `cluster_column`
...
#'   (mean-centered) matrix - scType scores are not meaningful on raw counts.
...
#'   for SEAAD reference labels or `"Donor.ID"` for QC). Colors are
#'   auto-generated from `Nour_pal()` unless supplied via `extra_col_colors`.
#' @param extra_col_colors Named list of color vectors for `extra_dimplot_cols`.
#'   Names must match `extra_dimplot_cols`. Example:
#'   `list(Subclass = subclass_colors)`.
#'
#' @section Dotplot scale:
#' @param col.min Numeric. Minimum scaled expression value for dotplot color
#'   scale. Default `0`.
#' @param col.max Numeric. Maximum scaled expression value for dotplot color
#'   scale. Default `2`.
#' @param scale.min Numeric. Minimum percentage of expressing cells for the
#'   dotplot size scale. Default `0`.
#' @param scale.max Numeric. Maximum percentage of expressing cells for the
#'   dotplot size scale. Default `100`.
#'
#' @section System:
#' @param caffeinate Logical. If `TRUE`, prevent macOS system sleep for the
#'   duration of the function call using `caffeinate`. Default `FALSE`.
#'
...
#'   \item{\code{seurat_object@misc$annotation_colors}}{Named list of color vectors for
#'     each annotation column: \code{rcellmarker}, \code{sctype},
#'     \code{singler}, plus any \code{extra_dimplot_cols} colors. Retrieve
...
CellTypeAssignmentHelper <- function(
  ...
  # Cluster colors: PrepObject stored → auto Nour_pal fallback
  ...
  if (run_rcellmarker) {
    ...
    tryCatch({
      # Normalize marker columns to what cellMarker(type="seurat") needs:
  ...
  # Each entry in dotplot_list is already a patchwork object from SplitDotPlot.
  {
    ...
    if (length(all_plot_list) > 0) {
      ...
      .write_legend_sidecar(f_helper, paste0(
        ...
        ". Dot size = % expressing cells; color = mean scaled expression."
      ))
    }
  }
