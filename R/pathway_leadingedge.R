...
VisualizeLeadingEdge <- function(
  ...
  # Helper - row annotation (coloured bar on the right, no legend - the row
  ...
  .write_legend_sidecar(pdf_all, paste0(
    hm_desc, " Leading-edge genes from GSEA pathways matching [", term_label, "]. ",
    "Color scale: blue = -2, white = 0, red = +2 (row-centered). ",
    "Genes with mean expression < ", min_expr, " excluded.",
    if (isTRUE(show_pathway_annotation) && n_pathways > 0)
      paste0(" Row annotation shows source pathway(s) per gene.") else "",
    if (!is.null(heatmap_column_split))
    ...
    if (nrow(deg_sig) > 0) {
      ...
      if (length(genes_top_hm) > 0) {
        ...
        .write_legend_sidecar(pdf_top, paste0(
          ...
          if (heatmap_type == "group") "Group-mean values per column." else
