...
PlotMetaSummary <- function(data,
  ...
  # ── 2. Validate columns ───────────────────────────────────────────────────
  all_needed  <- unique(c(id_column, variables, fill_variable, row_variable))
  ...
  if (percent) {
    if (!is.null(fill_variable)) {
      # Each x-axis bar sums to 100%: divide within (Variable, Value, row)
      ...
      # No fill: show each bar as % of the panel total (per Variable x row)
      total_cols <- "Variable"
      if (!is.null(row_variable)) total_cols <- c(total_cols, row_variable)
      agg_df <- agg_df |>
        dplyr::group_by(dplyr::across(dplyr::all_of(total_cols))) |>
  ...
  # ── 10. Build plot ─────────────────────────────────────────────────────────
  # Use direct column names in aes() - avoids dynamic .data[[]] lookups that
  # can hang when the returned plot is auto-printed outside function scope.
  if (is.null(fill_variable)) {
    var_colors <- SelectColors(agg_df$Variable, palette = "spectrum")
    p <- if (percent)
      ggplot2::ggplot(agg_df,
        ggplot2::aes(x = Value, y = pct, fill = Variable)) +
      ggplot2::geom_bar(stat = "identity", alpha = alpha) +
      ggplot2::scale_fill_manual(values = var_colors, name = "Variable")
    else
      ggplot2::ggplot(agg_df,
        ggplot2::aes(x = Value, y = count, fill = Variable)) +
      ggplot2::geom_bar(stat = "identity", alpha = alpha) +
      ggplot2::scale_fill_manual(values = var_colors, name = "Variable")
  } else {
    p <- if (percent)
      ggplot2::ggplot(agg_df,
        ggplot2::aes(x = Value, y = pct,
  ...
  p <- p + ggplot2::labs(x = "Value", y = y_label)

  # ── 11. Count / percent labels ────────────────────────────────────────────
  if (show_counts) {
    label_df <- agg_df[agg_df$count >= min_count_to_label, ]

    if (!is.null(fill_variable)) {
      # Middle of each stacked segment - direct column reference in aes()
      p <- p + ggplot2::geom_text(
        data     = label_df,
        ggplot2::aes(label = if (percent) paste0(round(pct, 1), "%") else count),
  ...
  # ── 12. Facets ─────────────────────────────────────────────────────────────
  facet_formula <- if (!is.null(row_variable))
  ...
  p <- p +
    ...
    ggplot2::theme(
      ...
      # theme_NourMin blanks all axis titles; explicitly restore the y title
  ...
  if (!is.null(output_dir)) {
    ...
    # ── Compute PDF dimensions from panel counts and x-tick counts ───────────
    # Width: sum across variables of (n_unique_values × 0.35 in), min 1.5 in/panel
    #        + 2.5 in for y-axis label + legend
    n_x_ticks <- vapply(variables, function(v) {
      length(unique(as.character(meta[[v]])))
    }, integer(1))
    panel_widths <- pmax(n_x_ticks * 0.35, 1.5)
    pdf_w <- sum(panel_widths) + 2.5

    # Height: n_row_facets × 3 in + 1.2 in for rotated x labels + strips
    n_row_facets <- if (!is.null(row_variable))
      length(unique(as.character(meta[[row_variable]])))
    else 1L
    pdf_h <- n_row_facets * 3 + 1.5

    # Build filename - file_name (verbatim) wins; otherwise auto-deduce from
    # object_name + fill/variable names so different runs don't overwrite.
    ...
    .write_legend_sidecar(fpath, paste0(
      "Stacked bar chart summarizing metadata distributions across ",
      y_label, ". ",
      "Each column panel shows a separate metadata variable (",
      paste(variables, collapse = ", "), "). ",
      if (!is.null(fill_variable))
        paste0("Bars are stacked and colored by ", fill_variable,
               ", with each segment labeled with its count. ")
      else
        "Each bar represents the total count per category. ",
  ...
  # ── 15. Assemble output ────────────────────────────────────────────────────
  # If mixed numeric + categorical: always return a list so both plots surface
  has_numeric <- !is.null(numeric_plot)

  if (!return_data && !return_flextable && !has_numeric) return(p)

  result <- list(
    categorical_plot = p,
    numeric_plot     = numeric_plot   # NULL when no numeric variables
  )
