...
#' alluvials that preserves your own colour palettes and theme.
#'
...
#'   Stream colour is determined by the first axis.
...
#'   Default \code{0.1}.
#' @param group_colors Named character vector of colours for the first axis
#'   (streams + left axis).  Auto-assigned if \code{NULL}.
#' @param split_colors Named character vector of colours for the last axis.
#'   Auto-assigned if \code{NULL}.
#' @param axes_colors Named \code{list} of colour vectors, keyed by column
...
PlotAlluvial <- function(
  ...
  # When the same value appears in multiple axes, the first axis's colour wins
  ...
  .one_panel <- function(meta_sub, panel_title = NULL) {
    ...
    # ── Label positioning: left for axis-1, right for axis-N, centre middle ──
    ...
    p <- ggplot2::ggplot(pdata, ggplot2::aes(x, id = id, split = y, value = n)) +
      ...
      ggplot2::theme(
        ...
        axis.text.x      = ggplot2::element_text(face = "bold", colour = "black", size = 13),
...
#'
#' @description
#' Visualises the composition of \code{stat.by} within each level of
#' \code{group.by} as a polar bar (rose) chart.  Each petal of the rose
#' corresponds to a \code{group.by} level; the petal is split by colour into
...
#'
#' @param data A Seurat object or a plain \code{data.frame}/\code{tibble}.
#' @param stat.by Character. Variable whose levels become the fill colour
...
PlotRose <- function(
  ...
  p <- ggplot2::ggplot(ct, ggplot2::aes(
    ...
    ggplot2::theme(
      ...
      plot.title    = ggplot2::element_text(face = "bold", hjust = 0.5),
      axis.text.x   = ggplot2::element_text(face = "bold", colour = "black", size = 8),
...
PlotChord <- function(
  ...
  # par(fig=...) must be set BEFORE this is called; circos.clear() resets only
  # circlize's internal state, not base-R par() settings.
  .draw_one <- function(mat_one, panel_title = NULL) {
    ...
    # Panel title drawn in the canvas coordinate space (above the circle)
    if (!is.null(panel_title) && nzchar(panel_title))
      graphics::text(0, 1 + canvas_padding * 0.85, labels = panel_title,
                     cex = 1.1, font = 2, adj = c(0.5, 0.5))
    circlize::circos.clear()
  }
  ...
  # Draw all diagrams in a grid using par(fig=..., new=TRUE)
  # Works for both single and multi-panel; for a single panel fig = c(0,1,0,1).
  .draw_grid <- function() {
    ...
    for (i in seq_len(n_plots)) {
      ...
      x1 <- col_i       / ncol_f
      x2 <- (col_i + 1) / ncol_f
      # y runs top-to-bottom in par(fig): row 0 = top
      y2 <- 1 - row_i       / nrow_f
      y1 <- 1 - (row_i + 1) / nrow_f

      # new = TRUE for all but the first panel - stays on same page
      graphics::par(fig = c(x1, x2, y1, y2), new = (i > 1L))
