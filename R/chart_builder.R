...
#'   containing `chart_builder.html`). Defaults to the bundled copy inside the
#'   scSidekick package (`inst/Chart_Builder`), or
#'   `getOption("nourkit.chart_builder_dir")` if set.
...
ChartBuilder <- function(data,
                         app_dir  = getOption(
                           ...
                           system.file("Chart_Builder", package = "scSidekick")
                         ),
                         filename = NULL,
                         max_rows = NULL,
                         launch   = TRUE) {
