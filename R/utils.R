...
.heatmap_pdf_dims <- function(body_w_in, body_h_in,
  ...
  # Legend allowance goes on the side the legend is actually drawn (left/right
  # consume horizontal space, top/bottom consume vertical). Reserving it on the
  # correct side is what keeps row names from ever overlapping the legend.
  leg_left  <- if (identical(legend_side, "left"))       legend_in else 0
  leg_right <- if (identical(legend_side, "right"))      legend_in else 0
  leg_vert  <- if (legend_side %in% c("top", "bottom"))  legend_in else 0

  # Width
  left_in  <- margin_in + extra_left_in + leg_left +
              (if (identical(row_names_side, "left"))  rn_space else 0)
  right_in <- margin_in + leg_right + extra_right_in +
              (if (identical(row_names_side, "right")) rn_space else 0)
  ...
  height_in <- body_h_in + top_in + bottom_in + leg_vert
