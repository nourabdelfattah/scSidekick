...
.heatmap_pdf_dims <- function(body_w_in, body_h_in,
  ...
  # Width
  left_in  <- margin_in + (if (identical(row_names_side, "left"))  rn_space else 0)
  right_in <- margin_in + (if (identical(row_names_side, "right")) rn_space else 0) +
              legend_in + extra_right_in
  width_in <- body_w_in + left_in + right_in + cn_overhang

  # Height
  top_in    <- margin_in + title_in + n_top_anno * 0.25 +
