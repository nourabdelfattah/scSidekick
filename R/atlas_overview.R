...
#' and optionally normal-tissue types) are placed in the centre.
#'
...
#'   still counted in the centre statistics.  Typical use:
...
#'   \code{0.5}.
#' @param y_inner Numeric.  Inner y edge of the wedge ring (top of the centre
#'   hole).  Default \code{-1}.
...
#' @param y_center_title Numeric.  y position of the centre title annotation.
#'   Default \code{-5.5}.
#' @param y_center_stats Numeric.  y position of the centre statistics.
#'   Default \code{-13}.
...
#'   Default \code{0.9}.
#' @param title_size Numeric.  Font size of the centre title.  Default
#'   \code{7}.
#' @param stats_size Numeric.  Font size of the centre statistics text.
#'   Default \code{5}.
#' @param title_color Character.  Colour for the centre title and circle
...
PlotAtlasWheel <- function(
  ...
  # ── All group levels (for colour assignment - consistent across facets) ────
  ...
  if (!is.null(output_dir)) {
    ...
    .write_legend_sidecar(fpath, paste0(
      ...
      if (!is.null(study_by)) paste0(" with contributing studies (", study_by, ") labelled inside each wedge") else "",
      ". Wedge size is uniform; study names and summary statistics are shown in the centre.",
      if (!is.null(patient_by)) paste0(" Donor counts are based on unique ", patient_by, " values.") else "",
      if (!is.null(facet.by))
        paste0(" Separate wheels shown for each level of ", facet.by, ".")
      else "",
